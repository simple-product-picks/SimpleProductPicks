# Best USB-C Car Charger UK 2026

Every car charger quotes a wattage. The number most people never check is the fuse behind the cigarette lighter socket. Ratings vary by car, commonly 10A to 20A, and at 12 volts that caps the socket somewhere between roughly 120 and 240 watts. Only the highest-output pick here can pass the bottom of that range, and only with several devices drawing at once. The picks below are ordered by how much of that power budget they fill: one that keeps things simple, one that splits the current across two ports, and one that tries to run the whole car off four.

## Top picks

### 1. Anker 75W USB C Car Charger (Retractable Cable) - Best Overall

**SPP Score: 8.5 / 10** - 75W max with built-in retractable USB-C cable, ultra-compact. Our all-round pick.

Scorecard: Capability 8.5/10 - Versatility 7.6/10 - Build 9.4/10 - Value 8.9/10

What keeps it off a 10: The built-in cable serves one device, so a passenger's phone queues behind yours, and the retracting mechanism can wear out.

Anker lists this as a 75W car charger with its USB-C cable built into the body and a retractable mechanism that reels it back in when you unplug. The cable is the point here. There's nothing to forget, nothing to untangle from the glovebox, and no spare lead rattling around the centre console. 75W is more than any phone draws. The built-in cable does mean one device at a time, though. If the passenger's phone also needs a charge, you will be passing yours back.

**Pros**
- ✓ Built-in retractable USB-C cable with nothing extra to buy or lose
- ✓ 75W is more than any phone draws, and Anker lists iPad and Galaxy support alongside iPhone
- ✓ Ultra-compact body sits low in the socket without blocking neighbouring controls

**Cons**
- ✗ One built-in cable, one device; the passenger's phone queues behind yours
- ✗ The retractable mechanism is one more thing that can wear out over time

**Who it's for:** Solo drivers who want to plug in, charge and reel the cable away without thinking about it.
**Who it's not for:** Families or anyone who regularly charges two devices on the same journey.

[Check price](https://www.amazon.co.uk/dp/B0D9NPDLG3?linkCode=ll2&tag=souvlakia-21)

---

### 2. INIU 66W Car Charger (USB-C 36W + USB-A 30W) - Best Budget

**SPP Score: 8.3 / 10** - 66W total (USB-C 36W plus USB-A 30W), PD3.0, 12V cigarette lighter, metal adapter. Our value pick.

Scorecard: Capability 8.3/10 - Versatility 7.8/10 - Build 8.3/10 - Value 8.7/10

What keeps it off a 10: Its USB-C port takes only 36W of the total, which bites if you plug in something hungrier, and no leads are included.

INIU's 66W car charger splits its output across two ports: a USB-C at 36W and a USB-A at 30W. That split is the practical advantage over the Anker above. You can charge a phone on the USB-C port and a dashcam, sat-nav or passenger's older device on the USB-A port at the same time without swapping cables. INIU lists PD3.0 support on the USB-C side, and the charger plugs into the standard 12V cigarette lighter socket. The body is metal, which should help it shed heat in a warm cabin. It doesn't bring its own cable, though. Bring your own USB-C lead, and bring a USB-A one if the second port needs to earn its keep.

**Pros**
- ✓ Two ports (USB-C plus USB-A) charge a phone and a dashcam or second device at once
- ✓ PD3.0 on the USB-C side for fast charging on phones that support it
- ✓ Metal body helps manage heat in the 12V socket

**Cons**
- ✗ No cable included, so you need your own USB-C and USB-A leads
- ✗ 36W on the USB-C port is the lower share of the total, which matters if you are charging something that draws more

**Who it's for:** Drivers running a phone and a dashcam, or anyone who wants two ports without spending more.
**Who it's not for:** Laptop users who need a single high-wattage port to keep a machine running on a long drive.

[Check price](https://www.amazon.co.uk/dp/B0CB1D68V4?linkCode=ll2&tag=souvlakia-21)

---

### 3. UGREEN 150W USB C Car Charger PD140W 4-Port - Best Premium

**SPP Score: 8.9 / 10** - 150W total PD140W (USB-C), 4 ports, cigarette lighter adapter. Our pick for maximum capability.

Scorecard: Capability 9.8/10 - Versatility 8.0/10 - Build 8.5/10 - Value 8.9/10

What keeps it off a 10: 150W can push close to or past the fuse limit on some cars, so check your cigarette lighter's fuse rating first.

UGREEN lists this car charger at 150W total with PD at 140W across four ports. That is the highest output here by some distance, and the four ports make it the only pick that can charge a laptop, a phone, a tablet and a dashcam from one cigarette lighter socket. Here's where the fuse context matters, though. A 150W draw sits within what a 15A fuse on a 12V circuit allows, but above the limit of a 10A fuse that some cars use on the cigarette lighter. You can check your car's fuse rating in the owner's manual or on the fuse box cover. UGREEN does not claim the charger will blow any fuses; it will draw only what connected devices ask for. But if you plug a hungry laptop into one port and a phone into another, the total draw can climb higher than what most of the lighter-duty chargers here would ever see.

**Pros**
- ✓ Four ports and 150W total means laptop, phone, tablet and dashcam can share one socket
- ✓ PD140W is the highest single-port output here, per UGREEN's listing
- ✓ Genuinely useful step up for drivers who work from a parked car between meetings

**Cons**
- ✗ With 140W going to one port, the other three share what is left of 150W
- ✗ The largest body of the three, and it sits deeper in the socket

**Who it's for:** Drivers who charge a laptop from the car, or anyone regularly powering three or four devices at once.
**Who it's not for:** Phone-only users, who would be paying for ports and watts that sit idle every journey.

[Check price](https://www.amazon.co.uk/dp/B0CPF4RDFQ?linkCode=ll2&tag=souvlakia-21)

---

## Quick verdict

For most UK drivers, the **[Anker 75W USB C Car Charger (Retractable Cable)](https://www.amazon.co.uk/dp/B0D9NPDLG3?linkCode=ll2&tag=souvlakia-21)** is the one to have: a retractable cable built in, 75W, and nothing else to think about. Drivers who run a dashcam or charge a passenger's phone at the same time should look at the **[INIU 66W Car Charger (USB-C 36W + USB-A 30W)](https://www.amazon.co.uk/dp/B0CB1D68V4?linkCode=ll2&tag=souvlakia-21)**, which adds a USB-A port for less money. If the car doubles as a mobile office and you charge a laptop between stops, the **[UGREEN 150W USB C Car Charger PD140W 4-Port](https://www.amazon.co.uk/dp/B0CPF4RDFQ?linkCode=ll2&tag=souvlakia-21)** is the step up, with enough output to run all of it from one socket. For wireless MagSafe charging on the vent instead, we cover that separately in the [MagSafe car charger guide](best-magsafe-car-charger-uk.html).

## How we chose

We started with what actually limits a car charger: the 12V cigarette lighter socket and the fuse behind it. Fuse ratings vary by car, commonly 10A to 20A, so the real-world ceiling is roughly 120 to 240 watts regardless of what the charger is rated for, and your owner's manual has the figure for yours. With that ceiling in mind, we picked chargers that cover three different load shapes: a single phone (the Anker), a phone plus a second device (the INIU), and a laptop plus everything else (the UGREEN). We checked every link against the live Amazon UK listing the day we wrote this, scored each pick on the criteria we publish, and left port counts and wattage claims as the makers list them. We take no payment for placement and we do not display any data we have not verified ourselves. If you need a [cable for the USB-C port](best-usb-c-cable-for-fast-charging.html), our cable guide covers that end.

## FAQ

**Q: Can a car charger damage my phone?**
A: No. USB-C Power Delivery negotiates between the charger and the device, so the phone draws only what it asks for. The charger's rated wattage is the ceiling, not what it pushes into every connected device.

**Q: What fuse does my car's cigarette lighter use?**
A: It varies by make and model. Check your owner's manual or the label on the fuse box cover, usually under the dashboard or in the boot. Common ratings are 10A, 15A or 20A. At 12 volts, a 10A fuse gives roughly 120W before it blows, and a 15A fuse gives roughly 180W.

**Q: Can I charge a laptop from a car charger?**
A: Some laptops, yes. You need a charger that delivers enough watts for the laptop's minimum draw, and a USB-C cable rated to carry it. The UGREEN here lists PD140W, which covers most USB-C laptops. Check your laptop's rated input before relying on it.

---

*Affiliate disclosure: This page contains affiliate links. As an Amazon Associate we earn from qualifying purchases, at no extra cost to you. See our [disclosure page](https://simpleproductpicks.co.uk/disclosure.html) for details.*
